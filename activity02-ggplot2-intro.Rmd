...
**Response**: The boxplots become different colors, each species is assigned a color
...
**Response**: From a boxplot we can easily identify the five number summary, as well as a general distribution. From a jitterplot be can more easily see the density of certain data points.
...
**Response**: The md file is much easier to read than an rmd file because it better uses the md language.
...
**Response**: Having the jitterplot laid over the boxplot is better, unless you lower the alpha value of the boxplot so it does not block out chunks of the boxplot.
...
**Response**: Mapping everything in ggplot changes the layout of geom_jitter, but coloring can remain the same.
...
**Response**: Sex appears to be the categorical variable that contributes to both peaks the most. Females tend to have shorter beaks than males.
...
**Response**: It appears that as flipper length increases, body mass increases.
...
**Response**: Adelie and Chinstrap appear to have similar distibutions and relationships. Gentoo has a greater positive relationship between flipper length and body mass as well as greater of the two in general.
...
**Response**: We are now able to see the density of different points, as well as if two different species overlap.
